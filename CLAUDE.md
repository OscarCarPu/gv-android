...
- **Tasks logic is pure and ported from gv-web** (`dueSoonGrouping.ts`, `planOverlay.ts`, `taskTree.ts`): keep the two in step, and when one changes, change the other.
- JVM unit tests live in `app/src/test/` and run with `./gradlew testFullDebugUnitTest`. They
  ...
  `PlanTimelineTest`, `PlanEditingTest`) pin the same kind of failure: an urgent task under "later",
